...
void ProtocolDecode(uint8_t *pData, uint32_t len)
{
	...
	uint32_t srcLen = SerialRead(bytes, 50);
	if(srcLen == 0)
		return;
...
//		}
		...
		// 处理解析后的数据
		SerialWrite(bytes, pBytes-bytes);
		...
		switch(bytes[0])
		{
			...
			case 0x04:	// 电机读取测速编码器
			{
				...
				SerialWrite((uint8_t*)&s, sizeof(s));
			}break;
			
			case 0x05:	// 复位MPU6050
			{
				uint8_t res = mpu_dmp_init();
				SerialWrite(&res, sizeof(res));
			}break;
