...
//    IIC_Stop();	 
	...
	return I2CWriteWithErrorHandle(&hi2c1, (addr<<1)|0, reg, I2C_MEMADD_SIZE_8BIT, buf, len, 100) != HAL_OK;
} 
//IIC连续读
//addr:器件地址
//reg:要读取的寄存器地址
...
//    IIC_Stop();	//产生一个停止条件 
	...
	return I2CReadWithErrorHandle(&hi2c1, (addr<<1)|1 , reg, I2C_MEMADD_SIZE_8BIT, buf, len, 100) != HAL_OK;
}
//IIC写一个字节 
//reg:寄存器地址
//data:数据
...
//    IIC_Stop();	 
	...
	return I2CWriteWithErrorHandle(&hi2c1, (MPU_ADDR<<1)|0, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 100) != HAL_OK;
}
//IIC读一个字节 
//reg:寄存器地址 
//返回值:读到的数据
...
//    IIC_Stop();			//产生一个停止条件 
	I2CReadWithErrorHandle(&hi2c1, (MPU_ADDR<<1)|1 , reg, I2C_MEMADD_SIZE_8BIT, &res, 1, 100);
	return res;		
}
